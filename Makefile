...
pkgdatadir = $(datadir)/xtella
pkgincludedir = $(includedir)/xtella
pkglibdir = $(libdir)/xtella
pkglibexecdir = $(libexecdir)/xtella
...
CONFIG_HEADER = $(top_builddir)/src/config/xtella-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/xtella-config.h.in \
...
ACLOCAL = ${SHELL} /home/chunche/xtella/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/chunche/xtella/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/chunche/xtella/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/chunche/xtella/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /home/chunche/xtella/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = xtella
PACKAGE_BUGREPORT = www.xtella.org
PACKAGE_NAME = xtella Core
PACKAGE_STRING = xtella Core 2.3.0
PACKAGE_TARNAME = xtella
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /home/chunche/xtella
abs_srcdir = /home/chunche/xtella
abs_top_builddir = /home/chunche/xtella
abs_top_srcdir = /home/chunche/xtella
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/chunche/xtella/build-aux/install-sh
...
SUBDIRS = src
GZIP_ENV = "-9n"
BITCOIND_BIN = $(top_builddir)/src/xtellad$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/xtella-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/xtella-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = xtella-Qt.app
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_xtella_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_xtella_coverage.info test_xtella.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/xtella-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/xtella-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/xtella-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/xtella-config.h
$(top_srcdir)/src/config/xtella-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/xtella-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/xtella-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/xtella-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/xtella-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_xtella.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_xtella -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_xtella_filtered.info: test_xtella.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_xtella_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_xtella_coverage.info: baseline_filtered_combined.info test_xtella_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_xtella_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_xtella_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_xtella_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_xtella.coverage/.dirstamp:  test_xtella_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_xtella.coverage/.dirstamp total.coverage/.dirstamp

# xtella: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_xtella.coverage/ total.coverage/ $(OSX_APP)
