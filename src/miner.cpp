...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if(prev->nHeight <= 9) { txNew.vout[0].nValue = 21000 * COIN;} // Premine 210,000 * 10 = 210,000 xtella 4.2%
    if (prev->nHeight > 9) { txNew.vout[0].nValue = 10 * COIN;} // Block 9 to infitnite pay 10 to prevent instamine
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("xtellaMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("xtellaMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("xtellaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xtella-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running xtellaMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
